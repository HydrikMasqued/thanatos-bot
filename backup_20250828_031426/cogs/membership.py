                ...
                if members_list:
                    ...
                    for member in members_list:
                        ...
                        if member.get('is_on_loa') or member.get('status') == 'LOA':
                            status_emoji = "🟠"  # Orange for LOA
                            status_text = "LOA"
                        else:
                            status_emoji = "🟢"  # Green for Active
                            status_text = "Active"
                        
                        # Format: Name (@username) - Status
                        username = member.get('discord_username', 'N/A')
                        name = member.get('discord_name', 'Unknown')
                        section_text += f"  {status_emoji} {name} (@{username}) - {status_text}\n"
                    section_text += "\n"
        
        if section_text:
            # Trim trailing newlines and check length
            section_text = section_text.strip()
            if len(section_text) > 1000:  # Discord field limit
                section_text = section_text[:950] + "... (truncated)"
