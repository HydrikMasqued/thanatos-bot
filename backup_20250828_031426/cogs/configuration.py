...
class ConfigurationSystem(commands.Cog):
    ...
    @app_commands.command(name="set_leadership_channel", description="Set the leadership channel for important notifications (Admin only)")
...
class UserSelectionView(discord.ui.View):
    ...
    async def user_select(self, interaction: discord.Interaction, select: discord.ui.Select):
        ...
        if user_id == "none":
            # Clear the user setting
            await self.bot.db.update_server_config(self.guild.id, dm_user_id=None)
            
            embed = await self.parent_view.create_main_embed()
            embed.add_field(
                name="✅ DM User Cleared",
                value="DM user setting has been cleared.",
                inline=False
            )
        else:
            user = self.guild.get_member(int(user_id))
            if user:
                await self.bot.db.update_server_config(self.guild.id, dm_user_id=user.id)
                
                embed = await self.parent_view.create_main_embed()
                embed.add_field(
                    name="✅ DM User Set",
                    value=f"DM user set to {user.mention}",
                    inline=False
                )
            else:
                embed = discord.Embed(
...
class RoleSearchModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if role_type not in ['officer', 'loa_notification']:
            await interaction.response.send_message(
                "❌ Invalid role type. Please use 'officer' or 'loa_notification'.", ephemeral=True
            )
            return
...
class ChannelSearchModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if channel_type not in ['notification', 'leadership', 'loa_notification']:
            await interaction.response.send_message(
                "❌ Invalid channel type. Please use 'notification', 'leadership', or 'loa_notification'.", ephemeral=True
            )
            return
